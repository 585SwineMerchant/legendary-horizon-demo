/**
 * Web App entry â€” deploy as "Execute as me" / "Anyone" (or domain) POST handler.
 ...
 * Copy these values from your live spreadsheet â€” never hardcode mystery columns in multiple files.
 *
...
var LH_SCHEMA = {
  ...
  /** Optional â€” quest definitions mirrored from Quest List workbook. */
  QUEST_DEFINITION_TAB: 'LhQuestDefinitions',
  /** Optional â€” realm registry rows. */
  REALM_DEFINITION_TAB: 'LhRealmDefinitions',
  /** Optional â€” item catalog for LookupService + teacher restore. */
...
var LH_PLAYER_SAVE_HEADERS = {
  ...
  /** M8 â€” JSON blobs mirroring `ManualSaveEnvelopeV1` optional slices. */
...
/** Media tab â€” core columns plus optional filters for AssetService.getRealmAssets / getNpcAssets. */
...
function LhAsset_getRecord(spreadsheetId, asset_id) {
  try {
    ...
    if (idCol === undefined) {
      throw new Error('asset_id column missing â€” align Media workbook');
    }
...
/**
 * ExitTicketService â€” Milestone 3 exit-ticket copy + state column + Gmail precursor.
 */
 ...
 */
/**
 * InterviewService â€” GT-102 "Trial of Tongues" backend turn runner.
 *
...
function lhInterview_buildReply_(turnIndex, npc, lastUserText) {
  // 0-based question index; keep replies short (2â€“3 sentences).
  if (turnIndex <= 0) {
    return (
      'I am ' +
      npc.name +
  ...
  if (turnIndex === 1) {
    return (
      'So noted. Tell me of a time you solved a problem using your unique skills â€” what did you do, and what was the outcome?'
    );
  }
  if (turnIndex === 2) {
...
/**
 * LookupService â€” Milestone 3 ID registries for validation (optional Sheets tabs).
 */
...
/**
 * @param {string} key â€” `quest_tier` | `quest_status` | `exit_ticket_state`
 */
...
/**
 * QuestService â€” Milestone 3 quest reads + snapshot mutations on player rows.
 */
...
/**
 * Roster identity resolution â€” correlates Classroom users with canonical `player_id`s.
 */
...
function LhRoster_resolvePlayerId(spreadsheetId, tabNameOverride, identity) {
  try {
    ...
    if (identity.student_email && emailIdx !== undefined) {
      ...
      if (rowMatch === -1) {
        // Case-insensitive search fallback â€” Sheets may already normalise casing.
        for (var i = 1; i < rows.length; i++) {
...
/**
 * SaveService â€” Milestone 3 persistence surface for player saves.
 ...
 * @param {object} patch â€” subset of player fields (inventory_summary merged replace if provided)
 */
 ...
 * @param {object} envelope â€” may include exploration_loop, realm_progress, session_summary, ritual_drafts
 */
...
function LhSave_readPlayerSave(spreadsheetId, tabNameOverride, playerId) {
  try {
    ...
    if (colIndex === undefined) {
      throw new Error('player_id column missing â€” align sheet to LhSheetSchema');
    }
...
/**
 * Writes the manual-save envelope back to the matching row (no backup snapshot â€” use `LhSave_manualSaveProgress` from clients).
 */
function LhSave_applyManualSaveEnvelope(spreadsheetId, tabNameOverride, envelope) {
  try {
    ...
    var rowIndex = lhSheetFindRowIndex_(rows, idCol, playerId);
    if (rowIndex === -1) {
      throw new Error('Cannot write save â€” player_id not found: ' + playerId);
    }
...
/**
 * SessionService â€” Milestone 3 class-block / play session history (append-only tab).
 */
...
/**
 * TeacherOverrideService â€” Milestone 3 educator rescue operations.
 */
...
/**
 * Rewinds narrative act index (does not auto-reset quests â€” pair with quest fixes as needed).
 */
...
/**
 * Generic sheet helpers â€” stay tiny & defensive for classroom reliability.
 */
